  ...
  <div class="main order-list-page">
    <div class="filter ly ly-j ly-m">
      <a
        ...
        @click="fetchList()"
        :class="{
          'filter-item-active': careStatus == null || careStatus == ''
        }"
        >全部</a
      >
      <a
        ...
        class="filter-item"
        @click="fetchList(1)"
        :class="{ 'filter-item-active': careStatus == 1 }"
        >待付款</a
      >
      <a
        href="javascript:void(0)"
        class="filter-item"
        @click="fetchList(2)"
        :class="{ 'filter-item-active': careStatus == 2 }"
        >待服务</a
      >
      <a
        href="javascript:void(0)"
        class="filter-item"
        @click="fetchList(3)"
        :class="{ 'filter-item-active': careStatus == 3 }"
        >待评价</a
      >
    </div>
    <div class="list">
          ...
          <div class="item__info">
            ...
            <div class="item__comment ly ly-r">
              <a
                href="javascript:void(0);"
                class="item__btn"
                @click="$router.push('/page/mall/order-comment/' + item.id)"
                v-if="item.status == 3"
                >匿名评价</a
              >
              <a
                href="javascript:void(0);"
                class="item__btn"
                @click="pay(item.id, item.moreInfo.careService.name)"
                v-if="item.status == 1"
                >付款</a
              >
              <a
