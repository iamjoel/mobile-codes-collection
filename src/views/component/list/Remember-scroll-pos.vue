  ...
  <div class="main">
    <section class="demo">
      <h2 class="demo__title">
        跳转其他页面后，返回进入该页面后，还返回上次浏览器的位置
      </h2>
      <van-list v-model="isLoading" :finished="isFinished" @load="fetchMore()">
        <van-cell-group>
          <van-cell
            v-for="item in list"
            :key="item.id"
...
export default {
  ...
  methods: {
    fetchList (isReset, pageLimit, succFn) {
      ...
      fetchList('singer', undefined, {
        ...
        if (
          this.pager.current >= Math.ceil(data.pager.total / this.pager.limit)
        ) {
  ...
  mounted () {
    ...
    if (this.$store.state.singerListCache.enable) {
      ...
      this.fetchList(
        true,
        singerListCache.pagerCurrent * this.pager.limit,
        () => {
